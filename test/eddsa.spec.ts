...
import {ED_CURVE, ED_CURVE_TO_DER_MARKER, Key} from "../src/constants"

describe("EdDSA", () => {
  let eddsa

  beforeEach(() => {
    eddsa = EdDSA.withCurve(ED_CURVE.ed25519)
  })
  ...
  test("sign and verify messages with Ed25519 - Test 1", () => {
    ...
    const eddsa = EdDSA.withCurve(ED_CURVE.ed25519).keyFromPrivate(privKey, "hex")
    ...
    EdDSA.withCurve(ED_CURVE.ed25519).keyFromPublic(pubKey, "hex")
  ...
  test("sign and verify messages with Ed25519 - Test 2", () => {
    ...
    let eddsa = EdDSA.withCurve(ED_CURVE.ed25519).keyFromPrivate(privKey, "hex")
    ...
    eddsa = EdDSA.withCurve(ED_CURVE.ed25519).keyFromPublic(pubKey)
    const verifyResult = eddsa.verify(message, signature)
  ...
  test("should convert a private key from and to PEM", () => {
    const eddsa = EdDSA.withCurve(ED_CURVE.ed25519).genKeyPair()
    ...
    let importedKey = Eddsa.withCurve(ED_CURVE.ed25519).fromPEM(pemPrivateKey).privateKey;
  ...
  test("should convert a public key from and to DER", () => {
    ...
    let importedKey = Eddsa.withCurve(ED_CURVE.ed25519).fromDER(derPrivateKey).privateKey;
    expect(eddsa.privateKey).toEqual(importedKey)
  })

  test("should correctly set prefix based on curve", () => {
    expect(eddsa.privateKeyPrefix).toBe(ED_CURVE_TO_DER_MARKER[ED_CURVE.ed25519][Key.privateKey])
    expect(eddsa.publicKeyPrefix).toBe(ED_CURVE_TO_DER_MARKER[ED_CURVE.ed25519][Key.publicKey])
  })
